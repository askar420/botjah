...
Ваш номер заказа %s
➖➖➖➖➖➖
📞 Тех. Поддержка: @valerx
➖➖➖➖➖➖
Оплата Easypay ➡️: 1488_228
➖➖➖➖➖➖
BitCoin ➡️ 33yPjjSMGHPp8zj1ZXySNJzSUfVSbpXEuL
➖➖➖➖➖➖
GLOBALMONEY ➡️ 1488_228
➖➖➖➖➖➖
После оплаты ID квитанции написать сюда. После оплаты клад будет выдан сразу!
Где находиться ID - http://prntscr.com/kspcml'''

region_text = '''❗️В случае ненахода - обязательно нужно иметь видеофиксацию до прихода на место, как вы начинаете путь. В противном случае - перезаклад рассматриваться не будет. Если есть проблемы с заказом, пожалуйста обратитесь к оператору @hawaii_oper2 или саппорту @valterx:
🌴 HAWAII WEED 🌴
